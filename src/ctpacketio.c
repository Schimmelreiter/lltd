...
validate_discover(void)
{
    ...
    {
        warn("verify_discover: frame with truncated Discover header (len=%ld src="
...
validate_hello()
{
    ...
    {
	warn("rx_hello: frame with truncated Hello header (len=%ld src="
	     ETHERADDR_FMT " dst=" ETHERADDR_FMT "); ignoring\n",
	     g_rcvd_pkt_len, ETHERADDR_PRINT(&g_ethernet_hdr->eh_src), ETHERADDR_PRINT(&g_ethernet_hdr->eh_dst));
        g_this_event.evtType =  evtInvalidPacket;
	return INVALID_PACKET;
...
validate_queryltlv()
{
    ...
    {
	warn("query-ltlv-rcvd: frame with truncated qltlv header (len=%ld src="
	     ETHERADDR_FMT " dst=" ETHERADDR_FMT "); ignoring\n",
	     g_rcvd_pkt_len, ETHERADDR_PRINT(&g_ethernet_hdr->eh_src), ETHERADDR_PRINT(&g_ethernet_hdr->eh_dst));
        g_this_event.evtType =  evtInvalidPacket;
	return INVALID_PACKET;
...
validate_emit()
{
    ...
    {
	warn("validate_emit: frame with truncated Emit header (len=%ld src="
	     ETHERADDR_FMT " dst=" ETHERADDR_FMT "); ignoring\n",
	     g_rcvd_pkt_len, ETHERADDR_PRINT(&g_ethernet_hdr->eh_src), ETHERADDR_PRINT(&g_ethernet_hdr->eh_dst));
        g_this_event.evtType =  evtInvalidPacket;
	return INVALID_PACKET;
...
tx_write(uint8_t *buf, size_t nbytes)
{
    ...
    if (ret != nbytes)
	die("packetio_tx_write: socket write for %ld bytes, but %ld went out\n",
	    nbytes, ret);
...
packetio_recv_handler(int fd, void *state)
{
    ...
    {
	warn("packetio_recv_handler: runt frame (%ld bytes < %ld); ignoring\n",
    ...
    if (g_rcvd_pkt_len < sizeof(topo_ether_header_t) + sizeof(topo_base_header_t))
    {
	warn("packetio_recv_handler: frame with truncated Base header (len=%ld src="
	     ETHERADDR_FMT " dst=" ETHERADDR_FMT "); ignoring\n",
	     g_rcvd_pkt_len, ETHERADDR_PRINT(&g_ethernet_hdr->eh_src), ETHERADDR_PRINT(&g_ethernet_hdr->eh_dst));
	return;
    }
